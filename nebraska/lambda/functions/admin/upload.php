...
function option_tree_upload($value, $settings, $int)
{
    ...
    <div class="option option-upload" id="<?php echo $value->item_id; ?>_div">
        ...
        <div class="section">
            <div class="element">
                <input type="text" name="<?php echo $value->item_id; ?>" id="<?php echo $value->item_id; ?>"
                       value="<?php if (isset($settings[$value->item_id])) {
                           echo $settings[$value->item_id];
                       } ?>" class="upload<?php if (isset($settings[$value->item_id])) {
                    echo ' has-file';
                } ?>"/>
                <button style="margin-left:0px; margin-top:10px;" id="upload_<?php echo $value->item_id; ?>"
                        class="upload_button btn btn-success" rel="<?php echo $int; ?>"/>
                    <i class="icon-picture icon-white"></i>
                    <?php _e('Upload', UT_THEME_NAME); ?>
                </button>
                ...
                <?php if (is_array(@getimagesize($settings[$value->item_id]))) { ?>
                    <div class="screenshot" id="<?php echo $value->item_id; ?>_image"
                         style="max-width:640px; overflow-x: hidden; ">
                        <?php
            ...
            <div class="desc alert alert-neutral">
